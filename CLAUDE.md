...
### Frontend (Vue 2 SPA)
- `src/router/index.js` — routes (`Landing`, `Home`, `Event`, `Settings`, `SignIn`/`Auth`, `PrivacyPolicy`, `PageNotFound` — see `src/views/`).
- `src/store/index.js` — single Vuex store (auth user, events, snackbar, dialogs).
- `src/components/` — organized by feature folder (`event/`, `home/`, `settings/`, `schedule_overlap/`, `calendar_permission_dialogs/`, `sign_up_form/`, `general/`) plus top-level shared components.
